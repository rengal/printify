...
public sealed partial class PrintersControllerTests
{
    ...
    private static async Task<DocumentDto> WaitForDocumentAsync(
        HttpClient client,
        Guid printerId,
        CancellationToken ct)
    {
        ...
        {
            var response = await client.GetAsync($"/api/printers/{printerId}/documents?limit=1", ct);
            if (response.IsSuccessStatusCode)
            {
                var list = await response.Content.ReadFromJsonAsync<DocumentListResponseDto>(cancellationToken: ct);
