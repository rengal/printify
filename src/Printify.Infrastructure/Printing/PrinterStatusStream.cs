...
public sealed class PrinterStatusStream : IPrinterStatusStream
{
    ...
    private readonly ConcurrentDictionary<Guid, ConcurrentDictionary<Guid, Channel<PrinterStatusUpdate>>> subscriptions =
        new();
    private readonly ConcurrentDictionary<Guid, Channel<PrinterStatusUpdate>> globalSubscriptions = new();
    ...
    public IAsyncEnumerable<PrinterStatusUpdate> SubscribeAll(CancellationToken ct)
    {
        ObjectDisposedException.ThrowIf(disposed, this);
        ...
        var subscriptionId = Guid.NewGuid();
        globalSubscriptions[subscriptionId] = channel;

        return ReadGlobalChannelAsync(subscriptionId, channel, ct);
    }
    ...
    public void Publish(Guid workspaceId, PrinterStatusUpdate update)
    {
        ...
        foreach (var channel in globalSubscriptions.Values)
        {
            try
            {
                channel.Writer.TryWrite(update);
    ...
    private async IAsyncEnumerable<PrinterStatusUpdate> ReadGlobalChannelAsync(
        Guid subscriptionId,
        ...
        [EnumeratorCancellation] CancellationToken ct)
    {
        ...
        finally
        {
            ...
            globalSubscriptions.TryRemove(subscriptionId, out _);
