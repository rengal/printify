...
$SshPort = 22
$SshKeyPath = ""
$LocalConfigPath = "deploy/nginx/printify.global.https.conf"
...
Invoke-Logged -Message "Uploading nginx config to server ($sshTarget)" -Action {
    $scpArgs = @()
...
Invoke-Logged -Message "Installing nginx config and reloading nginx" -Action {
    $remoteScript = @"
set -euo pipefail
...
Write-Host "Nginx config deployment completed."
